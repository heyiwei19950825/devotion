...
class WeiBo extends Controller
{
    ...
    public function getListByUid(){
        ...
        $listRow = json_decode(helper::http_curl($url.'&containerid=1076033920497465&page='.$page),true);

        if( $listRow['ok'] != 1 ){
            Log::record('微博数据获取失败 微博接口为：','error');
            return false;
        }else{
            Log::record($listRow['data'],'log');
        }


        //获取微博用户所发微博数据
        foreach( $listRow['data']['cards'] as $k=>$v ) {
            if( isset($v['mblog']) ){
                $pics = '暂无图片';
